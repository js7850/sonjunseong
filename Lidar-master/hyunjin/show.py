...
​
    ...
    img = np.ones([y_max + 1, x_max + 1], dtype=np.uint8)*255
...
​
    ...
    ax.scatter(x_img-XONES,YONES - y_img, s=2, c=pixel_values, linewidths=0, alpha=1, cmap=cmap)
    #ax.scatter(x_img,y_img, s=0.5, c=pixel_values, linewidths=0, alpha=1, cmap=cmap)
    #ax.set_facecolor((0, 0, 0)) # Set regions with no points to black
    ax.axis('scaled')              # {equal, scaled}
    #ax.xaxis.set_visible(False)    # Do not draw axis tick marks
    #ax.yaxis.set_visible(False)    # Do not draw axis tick marks
    plt.xlim([0,new_w ])   # prevent drawing empty space outside of horizontal FOV
