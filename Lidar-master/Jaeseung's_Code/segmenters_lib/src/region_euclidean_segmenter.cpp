...
void RegionEuclideanSegmenter::segment(
    ...
    *cloud = cloud_in;
    ...
    for (size_t pt = 0u; pt < cloud->size(); ++pt) {
        ...
        // warning zone
        if (cloud->points[pt].x <= 15. && cloud->points[pt].x >= 0. && cloud->points[pt].y < 1.5 && cloud->points[pt].y > -1.5 && cloud->points[pt].z < 1.) {
            ROS_INFO_STREAM("Warning : " << dist << " m");

			//TODO set brakeVal from ros param

			std_msgs::Int16 brakeVal;
			brakeVal.data = 14000;
			common::dbwPublish(common::brake_pub, brakeVal);
