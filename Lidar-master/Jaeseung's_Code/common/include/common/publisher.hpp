...
static void dbwPublish(const ros::Publisher &publisher, const std_msgs::Int16 &msg){
	ROS_INFO_STREAM("Publishing Actuator with " << msg.data);
...
static void publishObjectsMarkers(
    ...
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        ...
        end_point.x = center[0]; //end[0];
        end_point.y = center[1]; //end[1];
        end_point.z = center[2]; //end[2];
...
/*static void publishRoi(const ros::Publisher &publisher,
        ...
        visualization_msgs::MarkerArray empty_markers;
        visualization_msgs::Marker clear_marker;
        int total_size = 0; //total object in ROI
        clear_marker.header = header;
        clear_marker.ns = "objects";
        clear_marker.id = 0;
        clear_marker.action = clear_marker.DELETEALL;
        ...
        for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
            ...
            for (size_t i = 0u; i < 8; i++){
                if(bottom_quad[i](0) <= 50.0 && bottom_quad[i](0) >= -3.0 
                    && bottom_quad[i](1) <= 4.0 && bottom_quad[i](1) >= -4.0
                    && bottom_quad[i](2) <= 0.2 && bottom_quad[i](2) >= -2.73){
                    total_size++;
                    break;
                }
                if(i == 7){
        ...
        ROS_INFO(" %lu objects markers in ROI.", total_size);
        publisher.publish(object_markers);
}
*/
