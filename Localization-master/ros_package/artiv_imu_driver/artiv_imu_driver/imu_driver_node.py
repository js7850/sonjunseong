...
class GrabIMU380Data:
    def __init__(self, ws=False):
        ...
        self.node1 = rclpy.create_node("imu_error_node")
...
'''
        ...
        while not self.autobaud(self.find_ports()):
            ...
            time.sleep(1)
            if count == 3:
                self.node1.get_logger().fatal("IMU is NOT connected!")
                break
    ...
    def find_ports(self):
        ...
        if sys.platform.startswith('linux') or sys.platform.startswith('cygwin'):
            ...
            # I modified the code to scan 'artivIMU' only
            ports = glob.glob('/dev/artivIMU')
        else:
...
class ImuPublisherNode:
    def __init__(self):
        ...
        self.pub_roll = self.node.create_publisher(Float64, "/imu_roll")
        self.pub_pitch = self.node.create_publisher(Float64, "/imu_pitch")
        #self.pub_yaw = self.node.create_publisher(Float64, "/imu_yaw")
        ...
        self.node.get_logger().info("Ready for publishing IMU data")
    ...
    def publish_info(self, imu, node):
        ...
        self.rpy_roll.data = R
        self.rpy_pitch.data = P
