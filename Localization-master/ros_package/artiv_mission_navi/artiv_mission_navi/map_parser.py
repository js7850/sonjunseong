...
def protected_areas(location=default):
    if location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "A3_DRIVEWAYSECTION.osm")
    elif location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "A3_DRIVEWAYSECTION.osm")
...
# Mission Area (AUTHOR : JUNSANG RYU)
def mission_areas(location=default):
    if location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "A4_MISSIONSECTION.osm")
    elif location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "A4_MISSIONSECTION.osm")
    ...
    mission_ways = []
    for way in ways:
        mission_ways.append(way)

    return mission_ways

def mission_areas_polygons(location=default):
    if location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir + dgist + "A4_MISSIONSECTION.osm")
    elif location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir + k_city + "A4_MISSIONSECTION.osm")

    mission_areas_list = mission_areas(location)
    ...
    polygons = []
    for item in mission_areas_list:
        ...
        for node_id in temp_node_ref:
            for node in nodes:
                if node_id==node[1]:
                    temp_coords.append((node[3],node[2]))
        temp_poly = Polygon(temp_coords)
        polygons.append([temp_poly, item[4].get('ID'), item[4].get('missionType')])
...
def crosswalks(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "B3_SURFACEMARK.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "B3_SURFACEMARK.osm")
...
def speedbumps(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "C4_SPEEDBUMP.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "C4_SPEEDBUMP.osm")
...
def stoplines(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "B2_SURFACELINEMARK.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "B2_SURFACELINEMARK.osm")
...
def intersections(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "A1_NODE.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "A1_NODE.osm")
...
def traffic_signs(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "B1_SAFETYSIGN.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "B1_SAFETYSIGN.osm")
...
def traffic_lights(location=default):
    if location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "C1_TRAFFICLIGHT.osm")
    elif location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "C1_TRAFFICLIGHT.osm")
...
def guide_lines_linestrings(location=default):
    if location == 'dgist':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ dgist + "A2_LINK.osm")
    elif location == 'k_city':
        osm_data = artiv_mission_navi.OSMHandler.OSM_data(map_dir+ k_city + "A2_LINK.osm")
