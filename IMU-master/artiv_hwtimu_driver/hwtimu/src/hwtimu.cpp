...
ros::Publisher rpyimu_data_pub_;
ros::Publisher magimu_data_pub_;
ros::Publisher yawimu_data_pub_;
...
{
    node_handle_pri_.param("com_name",com_name_,std::string("/dev/artivIMU"));
    ...
    node_handle_pri_.param("loop_rate",loop_rate_,200.0);
    ...
    hwtimu_data_pub_ = node_handle_.advertise
                       <sensor_msgs::Imu>(imu_topic_,1);
    rpyimu_data_pub_ = node_handle_.advertise
                       <std_msgs::Float32MultiArray>("/imu/rpy_raw",1);
    magimu_data_pub_ = node_handle_.advertise
                       <sensor_msgs::MagneticField>("/imu/mag",1);
    yawimu_data_pub_ = node_handle_.advertise
                       <std_msgs::Float32>("/imu/yaw",1);
    while(ros::ok())
    {
        sleep(1);
        if(!openPort())
...
int Hwtimu::publish_imu_data()
{
    ...
    double sinr = +2.0*(orten[0]*orten[1] + orten[2]*orten[3]);
    double cosr = +1.0-2.0*(orten[1]*orten[1] + orten[2]*orten[2]);
    roll = std::atan2(sinr, cosr);

    // pitch (y-axis rotation)
    double sinp = +2.0*(orten[0]*orten[2]-orten[3]*orten[1]);
    if (std::fabs(sinp) >= 1)
        pitch = std::copysign(WYC_PI/2.0, sinp); // use 90 degrees if out of range
    else
        pitch = std::asin(sinp);

    // yaw (z-axis rotation)
    double siny = +2.0*(orten[0]*orten[3]+orten[1]*orten[2]);
    double cosy = +1.0-2.0*(orten[2]*orten[2]+orten[3]*orten[3]);  
...
/*
    double yaw_edited;
    if (-180.0 <= yaw && yaw <= 90.0)
    {
      yaw_edited = yaw + 90.0;
    }
    else
    {
      yaw_edited = yaw - 270.0;
    }
*/
    yaw_data.data = yaw;
    ...
    msg.magnetic_field.x = magn[0];
    msg.magnetic_field.y = magn[1];
    msg.magnetic_field.z = magn[2];

	// all covariance unknown
    for(int j=0;j<9;++j){
        msg.magnetic_field_covariance[j]=0;
    }

    hwtimu_data_pub_.publish(data);
    rpyimu_data_pub_.publish(array);
    magimu_data_pub_.publish(msg);
    yawimu_data_pub_.publish(yaw_data);
...
int Hwtimu::openPort()
{
  ...
  if(HwtimuCom.open(com_name_,baud_rate_,data_bits_,parity_,stop_bits_,mini_data_))
  {
    ...
    ROS_FATAL("type1 - Hwtimu::sampleFromPort:truncateData failed! RECOVERED");
    ROS_FATAL("type1 - Open port failed:%s RECOVERED",com_name_.c_str());
    sleep(1);
  }
  else
  {
    ...
    while(1)
    {
      ...
      if(re>0 && FD_ISSET(m_port_file, &readfds))
      {
        ...
        if(len >= 5*dataLen)
        {
          ...
          ROS_FATAL("type1 - Hwtimu::sampleFromPort:truncateData failed!");
